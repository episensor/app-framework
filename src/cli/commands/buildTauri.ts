...
export const buildTauriCommand = new Command("build-tauri")
  ...
  .option(
    ...
    "macos-arm64,win-x64,linux-x64",
  )
  .option("--no-compress", "Disable GZip compression")
  .option(
    "--externals <modules>",
    "Additional external modules (comma-separated)",
    "",
  )
  .option(
    "--generate-script",
    "Generate package.json script instead of building",
  )
  .action(async (options) => {
    try {
      const platforms = options.platforms.split(",").filter(Boolean);
      const externals = options.externals
        ? options.externals.split(",").filter(Boolean)
        : [];
