...
export class ConfigManager<T = any> {
    ...
    try {
      ...
      if (this.options.envPath) {
        ...
        if (result && result.error) {
          ...
          ensureLogger().debug(
            `Loaded environment from ${this.options.envPath}`,
          );
        }
      }
  ...
  private async loadJsonConfig(filePath: string): Promise<any> {
    try {
      ...
      if (!fs.existsSync(resolvedPath)) {
        ensureLogger().warn(
          `Config file not found: ${resolvedPath}, using defaults`,
        );
  ...
  private setupFileWatcher(): void {
    ...
    try {
      ...
      ensureLogger().warn(
        "Failed to setup config file watcher:",
        _error.message,
      );
    }
  }
  ...
  merge(partial: Partial<T>): void {
    ...
    if (this.schema) {
      ...
      if (!result.success) {
        throw new Error(
          `Invalid configuration after merge: ${result.error.message}`,
        );
      }
    }
...
export function createConfigManager<T>(
  options: ConfigOptions & { schema: ZodSchema<T> },
): ConfigManager<T> {
