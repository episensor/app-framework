...
router.get('/archives', async (_req: Request, res: Response) => {
  try {
    ...
    // Sort by modified date, newest first
    archives.sort((a, b) => new Date(b.modified).getTime() - new Date(a.modified).getTime());
...
router.get('/export', async (req: Request, res: Response) => {
  try {
    ...
    const textLogs = sortedLogs.map((log: LogEntry) => 
...
router.post('/compact', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: _error instanceof Error ? _error.message : String(_error) || 'Failed to compact logs' 
    });
  }
});
...
router.post('/cleanup-zero', async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: _error instanceof Error ? _error.message : String(_error) || 'Failed to cleanup logs' 
    });
  }
});
...
router.post('/purge-all', async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: _error instanceof Error ? _error.message : String(_error) || 'Failed to purge logs' 
    });
  }
});
...
router.get('/all-files', async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: _error instanceof Error ? _error.message : String(_error) || 'Failed to list log files',
      files: [] 
    });
  }
});
...
router.get('/download-any/:filename(*)', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ 
      success: false, 
      error: _error instanceof Error ? _error.message : String(_error) || 'Failed to download file' 
    });
  }
});
...
router.post('/rotate', async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: _error instanceof Error ? _error.message : String(_error) || 'Failed to rotate logs' 
    });
  }
});
