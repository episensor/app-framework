 ...
 */
export function formatLogEntry(entry: LogEntry, format: 'full' | 'compact' = 'full'): string {
  if (format === 'compact') {
...
export function filterLogEntries(entries: LogEntry[], filter: LogFilter): LogEntry[] {
  return entries.filter(entry => {
    ...
    // Filter by category
    if (filter.categories && filter.categories.length > 0 && entry.category && !filter.categories.includes(entry.category)) {
      return false;
    }
    ...
    if (filter.search) {
      ...
      const inMetadata = entry.metadata ? JSON.stringify(entry.metadata).toLowerCase().includes(searchLower) : false;
