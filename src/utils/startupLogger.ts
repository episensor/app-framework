...
class StartupLogger {
  ...
  banner(config: StartupConfig | null = null): void {
    ...
    if (!this.concise) {
      const appTitle =
        config?.get?.("app_title") || config?.app_title || "Device Simulator";
  ...
  summary(config: StartupConfig): void {
    ...
    const appTitle =
      config?.get?.("app_title") || config?.appTitle || "Device Simulator";
    const version = config?.get?.("version") || config?.version || "2.0.0";
    ...
    const isPreview = ["start", "start:api"].includes(
    ...
    const items = conflicts
      .map((c) => {
        ...
        if (c.process) {
          ...
          lines.push(
            `    ${chalk.gray("Process:")} ${c.process.command.substring(0, 80)}${c.process.command.length > 80 ? "..." : ""}`,
          );
          lines.push(
            `    ${chalk.gray("Kill:")} ${chalk.cyan(`kill -9 ${c.process.pid}`)}`,
          );
        } else {
          lines.push(
            `    ${chalk.gray("Kill:")} ${chalk.cyan(`lsof -ti:${c.port} | xargs kill -9`)}`,
          );
        }
        return lines.join("\n");
      })
