 ...
 */
function createErrorMessage(errors: FieldValidationError[], context: string): string {
  if (errors.length === 1) {
...
export const schemas = {
  ...
  template: z.object({
    ...
    data_points: z.array(z.object({
      ...
      simulation_type: z.enum(['static', 'random', 'csv', 'function']).optional(),
...
export function validate<T extends ZodSchema>(schema: T, options?: ValidationOptions) {
  ...
  return (req: Request, res: Response<ApiResponse<any>>, next: NextFunction): any => {
    try {
...
export const compose = {
   ...
   */
  require: <T extends z.ZodObject<any>>(schema: T, fields: Array<keyof T['shape']>) => {
   ...
   */
  pick: <T extends z.ZodObject<any>>(schema: T, fields: Array<keyof T['shape']>) => {
    const picked: any = {};
    fields.forEach(field => {
   ...
   */
  omit: <T extends z.ZodObject<any>>(schema: T, fields: Array<keyof T['shape']>) => {
    return schema.omit(
